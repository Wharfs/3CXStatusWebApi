...
- This is a small .NET 10 HTTP service (retargeted from .NET 8 in August 2026 — see parent CLAUDE.md) hosted on a 3CX v20 PBX server, exposing three endpoints (get extension profile, set one, set all) that wrap the legacy `3cxpscomcpp2.dll` COM Call Control API via `TCX.Configuration` / `TCX.PBXAPI`.
